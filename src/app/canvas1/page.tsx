...
export default function Canvas1Page() {
  ...
  // Automatically capture 3 images with 3-second intervals
  async function captureThreePhotos() {
    if (!streamActive) await startCamera();
    if (isCapturing || captures.length >= 3) return;

    setIsCapturing(true);
    setCaptures([]); // Clear previous captures

    // Capture 3 photos automatically
    for (let i = 0; i < 3; i++) {
      ...
      if (data) {
        setCaptures((prev) => {
          ...
          // Generate polaroid strip when all 3 photos are captured
          if (next.length === 3) {
  ...
  // Generate vertical polaroid strip from 3 photos
  async function generatePolaroidStrip(photos: string[]) {
    ...
    try {
      ...
      const totalHeight = (photoHeight * 3) + (dividerWidth * 2) + borderWidth + textHeight;
            ...
            try {
              ...
              if (ribbonImg.complete && ribbonImg.naturalWidth > 0) {
                try {
                  ...
                  // Determine position based on photo index
                  // Photo 1 (i=0): right corner, Photo 2 (i=1): left corner, Photo 3 (i=2): right corner
                  let ribbonX: number;
      ...
      const textY = borderWidth + (photoHeight * 3) + (dividerWidth * 2);
      ctx.fillStyle = "#000000";
  ...
  return (
    ...
    >
      ...
      {/* Counter */}
      <div className="counter">{captures.length}/3</div>

      {/* Main area */}
      <section className="mainArea">
        {polaroidStripUrl && captures.length === 3 ? (
        ...
        {!streamActive ? (
          ...
          <>
            <button
              ...
              disabled={isCapturing || captures.length >= 3}
            >
              {isCapturing ? "Capturing..." : captures.length >= 3 ? "Captured" : "Capture 3 Photos"}
            </button>
