...
export default function Canvas3Page() {
  ...
  // Automatically capture 2 images with 3-second intervals
  async function captureTwoPhotos() {
    if (!streamActive) await startCamera();
    if (isCapturing || captures.length >= 2) return;

    setIsCapturing(true);
    setCaptures([]); // Clear previous captures

    // Capture 2 photos automatically
    for (let i = 0; i < 2; i++) {
      ...
      if (data) {
        setCaptures((prev) => {
          const next = [...prev, data].slice(0, 2);
          // Generate polaroid strip when all 2 photos are captured
          if (next.length === 2) {
  ...
  // Generate vertical polaroid strip from 2 photos
  async function generatePolaroidStrip(photos: string[]) {
    ...
    try {
      ...
      const totalHeight = (photoHeight * 2) + dividerWidth + borderWidth + textHeight;
        ...
        await new Promise<void>((resolve, reject) => {
          img.onload = () => {
            try {
              ...
              // Photo 1 (i=0): right corner, Photo 2 (i=1): left corner
      ...
      const textY = borderWidth + (photoHeight * 2) + dividerWidth;
      ctx.fillStyle = "#000000";
  ...
  return (
    ...
    >
      ...
      {/* Counter */}
      <div className="counter">{captures.length}/2</div>

      {/* Main area */}
      <section className="mainArea">
        {polaroidStripUrl && captures.length === 2 ? (
        ...
        {!streamActive ? (
          ...
          <>
            <button
              ...
              onClick={captureTwoPhotos}
              disabled={isCapturing || captures.length >= 2}
            >
              {isCapturing ? "Capturing..." : captures.length >= 2 ? "Captured" : "Capture 2 Photos"}
            </button>
